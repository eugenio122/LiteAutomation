...
namespace LiteAutomation.Diagnostics
{
    /// <summary>
    /// Sistema de Log Local ultra-leve e à prova de falhas de permissão.
    /// Grava na pasta raiz do projeto (logs/liteautomation) para manter a portabilidade.
    /// </summary>
    public static class LiteLogger
    {
        ...
        static LiteLogger()
        {
            try
            {
                ...
                _logDirectory = Path.Combine(baseDir, "logs", "liteautomation");
                ...
                _logFilePath = Path.Combine(_logDirectory, $"LiteAutomation_{DateTime.Now:yyyyMMdd}.log");
            }
            catch
            {
        ...
        private static void WriteLog(string level, string message)
        {
            ...
            lock (_lock)
            {
                try
                {
                    ...
                    // FileShare.ReadWrite permite que o log seja escrito mesmo se o arquivo estiver bloqueado/sendo lido
