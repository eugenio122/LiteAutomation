    ...
    public class SeleniumBddPomJavaGenerator : ICodeGenerator
    {
        ...
        public string GenerateCode(WorkspaceState workspace, GeneratorConfig config, string testClassName = "GeneratedTest")
        {
            ...
            for (int i = 0; i < validIntents.Count; i++)
            {
                ...
                if (requiresLocator)
                {
                    string rawLocator = FormatJavaLocator(intent.TargetLocator);
                ...
                newAction.CodeBody = GeneratePomJavaActionCode(intent, locatorPropName, varName); activePage.Actions.Add(newAction);
            ...
            sb.AppendLine("// =====================================================================");
            sb.AppendLine($"// 👑 {LanguageManager.GetString("LblPattern").ToUpper()}: BDD POM HYBRID JAVA (4 LAYERS)");
            sb.AppendLine("// =====================================================================\n");
            ...
            sb.AppendLine("package liteautomation.generatedtests;\n");
            ...
            foreach (var page in pageDictionary.Values)
            {
                ...
                foreach (var action in page.Actions)
                {
                    ...
                    if (!string.IsNullOrEmpty(action.ErrorMsg)) { sb.AppendLine($"        try {{"); foreach (var line in action.CodeBody.Split('\n')) if (!string.IsNullOrWhiteSpace(line)) sb.AppendLine($"            {line.TrimEnd()}"); sb.AppendLine($"        }} catch (Exception e) {{\n            Assert.fail(\"{action.ErrorMsg}\");\n        }}"); }
            ...
            // 🚀 SETUP JAVA COM SEU ANTI-BOT EQUIVALENTE E NAVEGAÇÃO INICIAL
            sb.AppendLine($"\n    @Before\n    public void setup() {{");
            ...
            {
                sb.AppendLine($"\n        driver.get(\"{firstStepWithUrl.ObservedContext.Url}\");");
            }
            ...
            foreach (var step in stepDefinitions)
            {
                ...
                sb.AppendLine($"    @{javaAnnotation}(\"{step.StepDefText}\")\n    public void {char.ToLower(safeMethodName[0]) + safeMethodName.Substring(1)}({step.MethodParams}) {{");
        ...
        private string GeneratePomJavaActionCode(AutomationIntent intent, string propName, string varName)
        {
            string loc = string.IsNullOrEmpty(propName) ? "" : $"_page.{propName}";
            switch (intent.Type)
            {
                case IntentType.Click: return $"WebElement el = wait.until(ExpectedConditions.presenceOfElementLocated({loc}));\n((JavascriptExecutor)driver).executeScript(\"arguments[0].scrollIntoView({{block: 'center'}});\", el);\nwait.until(ExpectedConditions.elementToBeClickable(el));\ntry {{\n    el.click();\n}} catch (Exception e) {{\n    ((JavascriptExecutor)driver).executeScript(\"arguments[0].click();\", el);\n}}";
                case IntentType.InputText:
                    ...
                    return $"WebElement el = wait.until(ExpectedConditions.presenceOfElementLocated({loc}));\nel.clear();\n{sendKeysLine}";
                case IntentType.KeyPress: return $"WebElement el = wait.until(ExpectedConditions.presenceOfElementLocated({loc}));\nel.sendKeys({MapJavaKey(intent.Key)});\ntry {{ Thread.sleep(500); }} catch (InterruptedException e) {{ e.printStackTrace(); }}";
        ...
        private string FormatJavaLocator(string loc) { if (string.IsNullOrEmpty(loc) || loc.Contains("VAZIO") || loc.Contains("NÃO ENCONTRADO") || loc.Contains("AMBÍGUO")) return "By.tagName(\"body\")"; if (loc.StartsWith("By.") || loc.StartsWith("Page.")) return loc.Replace("By.CssSelector", "By.cssSelector").Replace("By.XPath", "By.xpath").Replace("By.Id", "By.id"); if (loc.StartsWith("xpath=") || loc.StartsWith("/") || loc.StartsWith("(")) return $"By.xpath(\"{loc.Replace("xpath=", "")}\")"; if (loc.StartsWith("css=")) return $"By.cssSelector(\"{loc.Replace("css=", "")}\")"; return $"By.cssSelector(\"{loc}\")"; }
        private string MapJavaKey(string key) => key?.ToLower().Replace("keypress_", "") switch { "enter" => "Keys.ENTER", "tab" => "Keys.TAB", "escape" => "Keys.ESCAPE", "space" => "Keys.SPACE", _ => $"\"{key}\"" };
